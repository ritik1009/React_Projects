  ...
  return (
    <div className="flex flex-wrap justify-between p-3 mt-2">
      {loading ? (
        ...
        data.map((item, idx) => {
          return (
            <Link to={`/detail/${item.id}`}><div className=" card shadow-xl hover:shadow-slate-400 p-1 md:p-2 cursor-pointer text-sm duration-300 font-small md:font-medium  mt-6">
              <img
                className="h-52 md:h-72 hover:scale-105 duration-300"
                src={item.img}
                alt=""
              />
              <h1 className="w-32 md:w-42 pt-2">
